...
function FormeEquipe({ onScoreSubmit }) {
  ...
  const handleCalculate = () => {
    if (equipe1.scores.length === 5 && equipe2.scores.length === 5) {
      onScoreSubmit(1, equipe1.nom, equipe1.scores);
      onScoreSubmit(2, equipe2.nom, equipe2.scores);
    ...
    setGoalAverageEquipe2(goalAverage2);
      ...
      // Réinitialiser la liste des scores
    setEquipe1({ ...equipe1, scores: [] });
    setEquipe2({ ...equipe2, scores: [] });
    // Réinitialiser l'état de calcul effectué
  ...
  return (
    ...
    <div className='form-container'>
      ...
      <div className='calculate'>
        <button onClick={handleCalculate} className='calculate-button'>Calculer</button>
      </div>
      ...
      {calculsEffectues && (
        <div className='results-container'>
          ...
          <p className='prediction-text'>{equipeFavorite !== 'Les deux équipes vont faire un jeu égal' ? (<>L'équipe favorite pour gagner le prochain match est : <strong>{equipeFavorite}</strong></>
          ) : (
